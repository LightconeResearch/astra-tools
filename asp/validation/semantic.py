...
Works with plain dicts loaded from YAML - no Pydantic models.
"""
...
def validate_analysis(data: dict[str, Any]) -> list[SemanticError]:
    ...
    Args:
        data: The analysis data as a dict (loaded from YAML).
    ...
    for out in analysis.get("outputs", []):
        out_id = out.get("id")
    ...
    for decision_id, decision in decisions.items():
        ...
        for option_id, option in options.items():
            ...
            # Check evidence refs
            evidence_list = option.get("evidence", []) if option else []
            ...
            if option:
                ...
                for ref in option.get("incompatible_with", []):
                    errors.extend(_validate_constraint_ref(ref, decisions, option_path))

                # Check requires refs
                for ref in option.get("requires", []):
                    errors.extend(_validate_constraint_ref(ref, decisions, option_path))
...
def validate_universe_file(
    ...
    analysis_data = _load_yaml(analysis_path)
