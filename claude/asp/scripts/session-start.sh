...
if ! command -v asp &> /dev/null; then
    ...
    echo "{\"hookSpecificOutput\": {\"hookEventName\": \"SessionStart\", \"additionalContext\": \"This is an ASP project. The asp CLI is not installed - run 'pip install -e .' from the ASP repo to enable validation and other commands.\"}}"
    exit 0
fi
...
# Check for CWL workflow
has_workflow="no"
if [ -f "workflows/main.cwl" ]; then
    has_workflow="yes"
fi
...
- CWL workflow: ${has_workflow}

Use the ASP skill (/asp) for working with this analysis. The skill provides guidance on editing asp.yaml, managing universes, extracting insights from papers, and building CWL workflows."
